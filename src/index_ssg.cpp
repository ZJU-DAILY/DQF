...
#define _CONTROL_NUM 100
  ...
  IndexSSG::IndexSSG(const size_t dimension, const size_t n, const size_t s_n, Metric m,
                     Index *initializer)
      : Index(dimension, n, s_n, m), initializer_{initializer}
  {
  }
  ...
  void IndexSSG::LoadWithSGraph(const char *filename, const char *s_filename)
  {
    std::ifstream in(filename, std::ios::binary);
    in.read((char *)&width, sizeof(unsigned));
    unsigned n_ep = 0;
    ...
    std::ifstream s_in(s_filename, std::ios::binary);
    s_in.read((char *)&s_width, sizeof(unsigned));
    unsigned s_n_ep = 0;
    s_in.read((char *)&s_n_ep, sizeof(unsigned));
    s_eps_.resize(s_n_ep);
    s_in.read((char *)s_eps_.data(), s_n_ep * sizeof(unsigned));
    // width=100;
    unsigned s_cc = 0;
    while (!s_in.eof())
    {
      unsigned k;
      s_in.read((char *)&k, sizeof(unsigned));
      if (s_in.eof())
        break;
      s_cc += k;
      std::vector<unsigned> tmp(k);
      s_in.read((char *)tmp.data(), k * sizeof(unsigned));
      s_final_graph_.push_back(tmp);
    }
    s_cc /= s_nd_;
    // std::cerr << "Average Small Degree = " << s_cc << std::endl;
  }
...
#pragma omp for schedule(dynamic, 100)
  ...
  void IndexSSG::BuildWithEps(size_t n, const float *data,
                              const Parameters &parameters, std::vector<unsigned> &eps)
  {
    ...
    DFS_expand_with_eps(parameters, eps);
                        ...
                        const Parameters &parameters, unsigned *indices)
  {
    ...
    // assert(eps_.size() < L);
    int cnt = 0;
    for (unsigned i = 0; i < std::min((unsigned)eps_.size(), L); i++)
    {
      init_ids[i] = eps_[i];
    }
    ...
    std::sort(retset.begin(), retset.begin() + L);
    // std::cout << "cnt: " << cnt << '\n';
    // for (auto neibo : retset)
    // {
    //   std::cout << sqrt(neibo.distance) << ' ';
    // }
    // std::cout << '\n';
    // cnt++;
    // _sleep(1000);
  ...
  unsigned IndexSSG::GetTrainData(const float *query, size_t K,
                                  const Parameters &parameters, unsigned *indices, unsigned *ids, std::vector<TrainData> &train_data)
  {
    unsigned L = parameters.Get<unsigned>("L_search");
    unsigned S_L = parameters.Get<unsigned>("S_L_search");
    ...
    std::vector<unsigned> init_ids(L);
    std::mt19937 rng(rand());
    GenRandom(rng, init_ids.data(), L, (unsigned)s_nd_);
    // assert(eps_.size() <= L);
    for (unsigned i = 0; i < std::min((unsigned)s_eps_.size(), L); i++)
    {
      init_ids[i] = s_eps_[i];
    }
    ...
    for (unsigned i = 0; i < init_ids.size(); i++)
    {
      ...
      if (id >= s_nd_)
        continue;
      _mm_prefetch(s_opt_graph_ + s_node_size * id, _MM_HINT_T0);
    }
    L = 0;
    for (unsigned i = 0; i < init_ids.size(); i++)
    {
      ...
      if (id >= s_nd_)
        continue;
      float *x = (float *)(s_opt_graph_ + s_node_size * id);
      float norm_x = *x;
      x++;
    ...
    while (k < (int)S_L)
    {
      ...
      if (retset[k].flag)
      {
        ...
        _mm_prefetch(s_opt_graph_ + s_node_size * n + s_data_len, _MM_HINT_T0);
        unsigned *neighbors = (unsigned *)(s_opt_graph_ + s_node_size * n + s_data_len);
        unsigned MaxM = *neighbors;
        neighbors++;
        for (unsigned m = 0; m < MaxM; ++m)
          _mm_prefetch(s_opt_graph_ + s_node_size * neighbors[m], _MM_HINT_T0);
        for (unsigned m = 0; m < MaxM; ++m)
        {
          ...
          if (flags[ids[id]])
            continue;
          flags[ids[id]] = 1;
          float *data = (float *)(s_opt_graph_ + s_node_size * id);
          float norm = *data;
          data++;
          float dist =
    ...
    {
      retset[i].flag = true;
      retset[i].id = ids[retset[i].id];
    }

    double query_index_dist_1st = retset[0].distance + query_norm;
    double query_index_dist_1st_div_kth = (retset[0].distance + query_norm) / (retset[K - 1].distance + query_norm);
    unsigned dist_count = 0;
    unsigned stop_search = 0;
    for (unsigned i = 0; i < eps_.size(); i++)
    {
      ...
      x++;
      float dist = dist_fast->compare(x, query, norm_x, (unsigned)dimension_);
      retset[pos] = Neighbor(id, dist, true);
      flags[id] = true;
    }
    ...
    k = 0;
    unsigned count = 0;
    unsigned freq = parameters.Get<unsigned>("freq");
      ...
      if (retset[k].flag)
      {
        ...
        for (unsigned m = 0; m < MaxM; ++m)
        {
          ...
          float dist =
              dist_fast->compare(query, data, norm, (unsigned)dimension_);
          dist_count++;
          if (dist_count % freq == 0)
          {
            TrainData td;
            td.query_index_dist_1st = sqrt(query_index_dist_1st);
            td.query_index_dist_1st_div_kth = sqrt(query_index_dist_1st_div_kth);
            td.dist_1st = sqrt(retset[0].distance + query_norm);
            td.dist_1st_div_kth = td.dist_1st / sqrt(retset[K - 1].distance + query_norm);
            td.dist_cnt = dist_count;
            td.update_cnt = update_count;
  ...
  void IndexSSG::SearchWithDtree(const float *query, size_t K,
    ...
    while (k < (int)L)
    {
      ...
      if (retset[k].flag)
      {
        ...
        for (unsigned m = 0; m < MaxM; ++m)
        {
          ...
          if (dist_count % freq == 0)
          {
            // use rtree to predict whether to continue searching
            // use six features
            cv::Mat sample(1, 6, CV_32F);
            sample.at<float>(0, 0) = sqrt(query_index_dist_1st);
            sample.at<float>(0, 1) = sqrt(query_index_dist_1st_div_kth);
            sample.at<float>(0, 2) = sqrt(retset[0].distance + query_norm);
            sample.at<float>(0, 3) = sqrt(retset[0].distance + query_norm) / sqrt(retset[K - 1].distance + query_norm);
            sample.at<float>(0, 4) = (float)dist_count;
            sample.at<float>(0, 5) = (float)update_count;
            float predict = dtree->predict(sample);
            if (predict > 0)
            {
              for (size_t i = 0; i < K; i++)
              {
  ...
  void IndexSSG::SearchWithoutDtree(const float *query, size_t K,
  ...
  void IndexSSG::SearchWithDtree_AddStep(const float *query, size_t K,
                                         const Parameters &parameters, unsigned *indices, unsigned *ids, cv::Ptr<cv::ml::DTrees> dtree)
  {
    unsigned L = parameters.Get<unsigned>("L_search");
    unsigned S_L = parameters.Get<unsigned>("S_L_search");
    int add_step = parameters.Get<unsigned>("add_step");
  ...
  void IndexSSG::SearchWithEps(const float *query, const float *x, size_t K,
                               const Parameters &parameters, unsigned *indices, unsigned *ep, std::vector<std::vector<unsigned>> &eps_set, unsigned K_small)
  {
    ...
    int cnt = eps_.size();
    int l = 0;
    while (cnt < L && l < K_small)
    {
      for (unsigned i = 0; i < eps_set[ep[l]].size() && cnt < L; i++)
      {
        init_ids[cnt++] = eps_set[ep[l]][i];
      }
      l++;
    }
  ...
  void IndexSSG::SearchForCandidate(const float *query, size_t K,
                                    const Parameters &parameters,
                                    std::vector<Neighbor> &indices, size_t be, boost::dynamic_bitset<> &flags_build)
  {
    ...
    boost::dynamic_bitset<> flags{s_nd_, 0};
    for (unsigned i = 0; i < init_ids.size(); i++)
    {
      unsigned id = init_ids[i];
      while (init_ids[i] >= be && flags_build[init_ids[i]] == 0)
      {
        init_ids[i] = rand() % s_nd_;
      }
      if (id >= s_nd_)
        continue;
      _mm_prefetch(s_opt_graph_ + s_node_size * id, _MM_HINT_T0);
    ...
    // std::cout << "retset size: " << retset.size() << std::endl;
    for (size_t i = 0; i < K; i++)
    {
      indices[i] = retset[i];
    }
  ...
  void IndexSSG::FreeOptimizeGraph()
  {
    if (opt_graph_)
    {
      free(opt_graph_);
    ...
    {
      free(s_opt_graph_);
      s_opt_graph_ = nullptr;
    }
  }

  void IndexSSG::OptimizeGraphWithSGraph(const float *data, const float *s_data)
  { // use after build or load
    data_ = data;
    data_len = (dimension_ + 1) * sizeof(float);
    neighbor_len = (width + 1) * sizeof(unsigned);
    ...
    s_data_ = s_data;
    s_data_len = (dimension_ + 1) * sizeof(float);
    s_neighbor_len = (s_width + 1) * sizeof(unsigned);
    s_node_size = s_data_len + s_neighbor_len;
    s_opt_graph_ = (char *)malloc(s_node_size * s_nd_);
    for (unsigned i = 0; i < s_nd_; i++)
    {
      char *cur_node_offset = s_opt_graph_ + i * s_node_size;
      float cur_norm = dist_fast->norm(s_data_ + i * dimension_, dimension_);
      std::memcpy(cur_node_offset, &cur_norm, sizeof(float));
      std::memcpy(cur_node_offset + sizeof(float), s_data_ + i * dimension_,
                  s_data_len - sizeof(float));

      cur_node_offset += s_data_len;
      unsigned k = s_final_graph_[i].size();
      std::memcpy(cur_node_offset, &k, sizeof(unsigned));
      std::memcpy(cur_node_offset + sizeof(unsigned), s_final_graph_[i].data(),
                  k * sizeof(unsigned));
      std::vector<unsigned>().swap(s_final_graph_[i]);
    }
    CompactGraph().swap(s_final_graph_);
  }

  void IndexSSG::OptimizeGraphWithSGraph_MoreSpace(const float *data, const float *s_data, unsigned more_space)
  { // use after build or load
    data_ = data;
    data_len = (dimension_ + 1) * sizeof(float);
    neighbor_len = (width + 1) * sizeof(unsigned);
    ...
    s_neighbor_len = (s_width + 1) * sizeof(unsigned);
    s_node_size = s_data_len + s_neighbor_len;
    s_opt_graph_ = (char *)malloc(s_node_size * more_space);
    locks.resize(more_space);
    for (auto &lock : locks)
    {
...
#pragma omp parallel for
  ...
  void IndexSSG::DFS_expand_with_eps(const Parameters &parameter, std::vector<unsigned> &eps)
  {
    unsigned n_try = parameter.Get<unsigned>("n_try");
    unsigned range = parameter.Get<unsigned>("R");
...
#pragma omp parallel for
  ...
  void IndexSSG::Insert(const float *s_data, size_t be, size_t ed,
                        const Parameters &parameters)
  {
    ...
    s_data_ = s_data;
    s_nd_ = ed;
    boost::dynamic_bitset<> flags_build{s_nd_, 0};
    #pragma omp parallel
    {
      std::vector<Neighbor> pool;
      #pragma omp for schedule(dynamic, 100)
      for (unsigned n = be; n < ed; ++n)
      {
        pool.clear();
        pool.resize(L);
        // std::cout << "SearchForCandidate: " << n << std::endl;
        SearchForCandidate(s_data_ + n * dimension_, L, parameters, pool, be, flags_build);
        // std::cout << "Insert: " << n << std::endl;
        insert_prune(n, pool, parameters, threshold);
        flags_build[n] = 1;
      }
    }
  }

  void IndexSSG::insert_prune(unsigned q, std::vector<Neighbor> &pool,
                              const Parameters &parameters, float threshold)
  {
    unsigned range = parameters.Get<unsigned>("R");
    width = range;
    ...
    while (result.size() < range && (++start) < pool.size())
    {
      ...
      for (unsigned t = 0; t < result.size(); t++)
      {
        ...
        float djk = distance_->compare(s_data_ + dimension_ * (size_t)result[t].id,
                                       s_data_ + dimension_ * (size_t)p.id,
                                       (unsigned)dimension_);
        float cos_ij = (p.distance + result[t].distance - djk) / 2 /
                       sqrt(p.distance * result[t].distance);
        if (cos_ij > threshold)
    ...
    unsigned *edge = new unsigned[result.size()];
    ...
    DistanceFastL2 *dist_fast = (DistanceFastL2 *)distance_;
    char *cur_node_offset = s_opt_graph_ + q * s_node_size;
    float cur_norm = dist_fast->norm(s_data_ + q * dimension_, dimension_);
    std::memcpy(cur_node_offset, &cur_norm, sizeof(float));
    std::memcpy(cur_node_offset + sizeof(float), s_data_ + q * dimension_,
                s_data_len - sizeof(float));

    cur_node_offset += s_data_len;
    unsigned k = result.size();
    std::memcpy(cur_node_offset, &k, sizeof(unsigned));
    std::memcpy(cur_node_offset + sizeof(unsigned), edge,
                k * sizeof(unsigned));

    for (size_t i = 0; i < k; i++)
    {
      unsigned des = edge[i];
      _mm_prefetch(s_opt_graph_ + s_node_size * des + s_data_len, _MM_HINT_T0);
      unsigned *neighbors = (unsigned *)(s_opt_graph_ + s_node_size * des + s_data_len);
      unsigned MaxM = *neighbors;
      neighbors++;
      SimpleNeighbor sn(q, 0);
      sn.distance = distance_->compare(s_data_ + q * dimension_,
                                       s_data_ + des * dimension_,
                                       (unsigned)dimension_);
      if (MaxM + 1 > range)
      {
        std::vector<SimpleNeighbor> temp_pool(MaxM + 1);
        for (unsigned m = 0; m < MaxM; ++m)
        {
          unsigned id = neighbors[m];
          float *data = (float *)(s_opt_graph_ + s_node_size * id);
          ...
          data++;
          float dist =
              dist_fast->compare(s_data_ + q * dimension_, data, norm, (unsigned)dimension_);
          temp_pool[m] = SimpleNeighbor(id, dist);
        }
        temp_pool[MaxM] = sn;
        std::sort(temp_pool.begin(), temp_pool.end());
        std::vector<SimpleNeighbor> result;
        ...
        while (result.size() < range && (++start) < temp_pool.size())
        {
          ...
          for (unsigned t = 0; t < result.size(); t++)
          {
            ...
            float djk = distance_->compare(
                s_data_ + dimension_ * (size_t)result[t].id,
                s_data_ + dimension_ * (size_t)p.id, (unsigned)dimension_);
            float cos_ij = (p.distance + result[t].distance - djk) / 2 /
                           sqrt(p.distance * result[t].distance);
            if (cos_ij > threshold)
            {
        ...
        {
          LockGuard guard(*locks[des]);
          unsigned *des_neighbors = (unsigned *)(s_opt_graph_ + s_node_size * des + s_data_len);
          *des_neighbors = result.size();
          des_neighbors++;
          for (unsigned t = 0; t < result.size(); t++)
          {
            des_neighbors[t] = result[t].id;
          }
        }
      }
      else
      {
        LockGuard guard(*locks[des]);
        unsigned *des_neighbors = (unsigned *)(s_opt_graph_ + s_node_size * des + s_data_len);
        *des_neighbors = MaxM + 1;
        des_neighbors++;
        des_neighbors[MaxM] = q;
      }
    }
    delete edge;
  }
